/**
 * OUTGOING API DEPENDENCIES — National Land Intelligence System
 *
 ...
 *
 * Criticality: normal. Uses `tryCall`, so if food-water is not
 ...
 */
export async function fromFoodWaterIrrigationPlan(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('food-water', 'GET /irrigation/plan', {
    ...options,
    relation: 'land -> food-water (GET /irrigation/plan)',
  });
}
