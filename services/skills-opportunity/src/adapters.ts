/**
 * OUTGOING API DEPENDENCIES — National Skills & Opportunity OS
 *
 ...
 *
 * Criticality: normal. Uses `tryCall`, so if research is not
 ...
 */
export async function fromResearchCapability(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('research', 'GET /capability', {
    ...options,
    relation: 'skills-opportunity -> research (GET /capability)',
  });
}
