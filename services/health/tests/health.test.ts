...
/**
 * Contract + relation tests for Connected Health Intelligence System.
 *
...
describe('health', () => {
  ...
  it('starts and answers /health', async () => {
    const response = await platform.get('health', '/health');
    expect(response.status).toBe(200);
    expect((response.body as { service: string }).service).toBe('health');
  });
  ...
  it('owns a contract for every event it declares', () => {
    for (const type of definition.routes.length
      ...
      expect(contract!.owner).toBe('health');
    }
  });

  it('publishes health.capacity.updated.v1 in a shape its consumers can read', async () => {
    const contract = eventContract('health.capacity.updated.v1')!;
  ...
  it('subscribes to all 14 events declared in architecture/relations.yaml', () => {
    ...
    for (const expected of [
      ...
      expect(subscribed.has(expected), `health does not consume ${expected}`).toBe(true);
    }
  });
  ...
  it('records a signal when a partner ministry publishes', async () => {
    ...
    const response = await platform.get('health', '/signals');
  ...
  it('degrades cleanly when a dependency is missing', async () => {
    const response = await platform.get('health', '/dependencies');
    expect(response.status).toBe(200);
