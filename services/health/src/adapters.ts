/**
 * OUTGOING API DEPENDENCIES — Connected Health Intelligence System
 *
 ...
 */
export async function fromMobilityLogisticsResourcesNearest(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('mobility-logistics', 'GET /resources/nearest', {
    ...options,
    relation: 'health -> mobility-logistics (GET /resources/nearest)',
  });
}
...
export async function fromEnvironmentAirQuality(
  ...
  return ctx.platform.tryCall('environment', 'GET /air-quality', {
    ...options,
    relation: 'health -> environment (GET /air-quality)',
  });
}
