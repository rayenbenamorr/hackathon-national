/**
 * OUTGOING API DEPENDENCIES — Smart Religious Heritage Network
 *
 ...
 *
 * Criticality: normal. Uses `tryCall`, so if culture is not
 ...
 */
export async function fromCultureAssetsCondition(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('culture', 'GET /assets/condition', {
    ...options,
    relation: 'religious-heritage -> culture (GET /assets/condition)',
  });
}
