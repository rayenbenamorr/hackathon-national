/**
 * OUTGOING API DEPENDENCIES — Global Tunisia Network
 *
 ...
 *
 * Criticality: normal. Uses `tryCall`, so if skills-opportunity is not
 ...
 */
export async function fromSkillsOpportunityGaps(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('skills-opportunity', 'GET /gaps', {
    ...options,
    relation: 'global-tunisia -> skills-opportunity (GET /gaps)',
  });
}
