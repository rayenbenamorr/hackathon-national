...
/**
 * Contract + relation tests for Life & Care Intelligence OS.
 *
...
describe('life-care', () => {
  ...
  it('starts and answers /health', async () => {
    const response = await platform.get('life-care', '/health');
    expect(response.status).toBe(200);
    expect((response.body as { service: string }).service).toBe('life-care');
  });

  it('owns a contract for every event it declares', () => {
    for (const type of definition.routes.length
      ? ['care.life-event.recorded.v1', 'care.support-need.detected.v1', 'care.facility-capacity.updated.v1']
      : []) {
      const contract = eventContract(type);
      expect(contract, `missing contract for ${type}`).toBeDefined();
      expect(contract!.owner).toBe('life-care');
    }
  });

  it('publishes care.life-event.recorded.v1 in a shape its consumers can read', async () => {
    const contract = eventContract('care.life-event.recorded.v1')!;
  ...
  it('subscribes to all 14 events declared in architecture/relations.yaml', () => {
    ...
    for (const expected of [
      ...
      expect(subscribed.has(expected), `life-care does not consume ${expected}`).toBe(true);
    }
  });

  it('records a signal when a partner ministry publishes', async () => {
    const relation = { event: 'social.vulnerability.updated.v1', from: 'social-mobility' };
    await platform.publish(relation.from, relation.event);
    const response = await platform.get('life-care', '/signals');
  ...
  it('degrades cleanly when a dependency is missing', async () => {
    const response = await platform.get('life-care', '/dependencies');
    expect(response.status).toBe(200);
