/**
 * OUTGOING API DEPENDENCIES — Life & Care Intelligence OS
 *
 ...
 *
 * Criticality: critical. Uses `tryCall`, so if social-mobility is not
 ...
 */
export async function fromSocialMobilityVulnerability(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('social-mobility', 'GET /vulnerability', {
    ...options,
    relation: 'life-care -> social-mobility (GET /vulnerability)',
  });
}
