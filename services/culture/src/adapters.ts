/**
 * OUTGOING API DEPENDENCIES — Tunisia Cultural Intelligence Network
 *
...
/**
 * Visitor pressure is read when a cultural event is planned.
 *
 * Criticality: normal. Uses `tryCall`, so if tourism is not
 ...
 */
export async function fromTourismFlows(
  ctx: ServiceContext,
  options: { query?: Record<string, string | number | boolean | undefined>; body?: unknown } = {},
) {
  return ctx.platform.tryCall('tourism', 'GET /flows', {
    ...options,
    relation: 'culture -> tourism (GET /flows)',
  });
}
