/**
 * POLICIES — RESERVED FOR PROMPT 02.
 *
 * Who may do what, and under which conditions.
 *
 ...
 * (@platform/policies), and the two shapes below so that wiring it in later
...
export interface PoliciesDecision {
  allowed: boolean;
  reason: string;
...
export function evaluate(_context: PoliciesContext): PoliciesDecision {
  return { allowed: true, reason: 'No policies are defined yet (reserved for Prompt 02).' };
}
